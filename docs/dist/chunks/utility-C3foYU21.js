import "./_init-DtYkg9NZ.js";
import { d as te, g as we, c as Ae, o as Te, f as ye, s as Ee, a as xe } from "./floating-CqSaT-kJ.js";
import { bl as B } from "./css-rF0v2sLP.js";
import { k as ke, e as Le, p as re } from "./rx-state-qVN2AgXP.js";
import { v as Se } from "./v4-CDGHvpHw.js";
...
function Ye(a) {
  ...
  let e = null;
  const { tooltipElement: u, updateText: d } = ke();
  function E(s) {
    return {
  ...
  const k = (s) => {
    const r = Le("[data-ir-tooltip]", ".ir-tooltip-container", s.target);
  ...
  return i.classList.add("ir-tooltip-container"), i.addEventListener("mousemove", k), i.addEventListener("mouseleave", t), {
    destroy() {
      e?.cleanup(), i.removeEventListener("mousemove", k), i.removeEventListener("mouseleave", t);
    },
    cleanup() {
      e?.cleanup();
    }
...
function Re() {
  ...
  return m.prototype.process = function(e) {
    ...
    var u = this, d = u.options, E = d.whiteList, S = d.onAttr, k = d.onIgnoreAttr, t = d.safeAttrValue, s = i(e, function(r, w, y, c, _) {
      var g = E[y], p = !1;
...
function ve() {
  ...
  function k(o, b, l, v) {
    if (l = U(l), b === "href" || b === "src") {
      ...
      if (p.lastIndex = 0, p.test(l) || (L.lastIndex = 0, L.test(l) && (g.lastIndex = 0, g.test(l))))
        return "";
      v !== !1 && (v = v || m, l = v.process(l));
    }
    return l = R(l), l;
  }
  var t = /</g, s = />/g, r = /"/g, w = /&quot;/g, y = /&#([a-zA-Z0-9]*);?/gim, c = /&colon;?/gim, _ = /&newline;?/gim, g = /((j\s*a\s*v\s*a|v\s*b|l\s*i\s*v\s*e)\s*s\s*c\s*r\s*i\s*p\s*t\s*|m\s*o\s*c\s*h\s*a):/gi, p = /e\s*x\s*p\s*r\s*e\s*s\s*s\s*i\s*o\s*n\s*\(.*/gi, L = /u\s*r\s*l\s*\(.*/gi;
  function h(o) {
    return o.replace(r, "&quot;");
  }
  function A(o) {
...
function he() {
  ...
  function f(t, s, r) {
    var w = "", y = 0, c = !1, _ = !1, g = 0, p = t.length, L = "", h = "";
    e: for (g = 0; g < p; g++) {
      var A = t.charAt(g);
      if (c === !1) {
        if (A === "<") {
        ...
        if (A === ">" || g === p - 1) {
          w += r(t.slice(y, c)), h = t.slice(c, g + 1), L = i(h), w += s(
            c,
            w.length,
            L,
            h,
    ...
    for (var p = 0; p < _; p++) {
      ...
      if (/\s|\n|\t/.test(L))
        if (t = t.replace(/\s|\n|\t/g, " "), c === !1)
          ...
          h = a.trim(t.slice(r, p)), h = k(h), g(c, h), c = !1, r = p + 1;
          continue;
        } else
          continue;
    }
    return r < t.length && (c === !1 ? g(t.slice(r)) : g(c, k(a.trim(t.slice(r))))), a.trim(y.join(" "));
  }
  function u(t, s) {
    for (; s < t.length; s++) {
      var r = t[s];
...
function Pe() {
  ...
  return k.prototype.process = function(t) {
    ...
    var s = this, r = s.options, w = r.whiteList, y = r.onTag, c = r.onIgnoreTag, _ = r.onTagAttr, g = r.onIgnoreTagAttr, p = r.safeAttrValue, L = r.escapeHtml, h = s.attributeWrapSign, A = s.cssFilter;
...
//# sourceMappingURL=utility-C3foYU21.js.map
