...
import {
  ...
  offset,
  shift
} from "./chunk-FWLV7CMV.js";
import {
  __commonJS,
  __toESM
} from "./chunk-NP7LZUD4.js";
...
function updateLoadingElements() {
  const context3 = getLoadingUpdateContext();
  context3.elements.forEach(([refElement, loadingElement]) => {
    updatePos(refElement, loadingElement);
  });
  context3.elements.length = 0;
  context3.scheduleHandler = -1;
}
function updatePos(refElement, loadingElement) {
...
function scheduleElement(refElement, loadingElement) {
  cancelAnimationFrame(getLoadingUpdateContext().scheduleHandler);
  getLoadingUpdateContext().elements.push([refElement, loadingElement]);
  getLoadingUpdateContext().scheduleHandler = requestAnimationFrame(updateLoadingElements);
}
var createLoadingHandler = ({ contextElement, onClick, startDelay = 0 }) => {
  const loading = createLoading();
  const loadingScrollHandler = () => {
    scheduleElement(contextElement, loading);
  };
  let scheduled = false;
  let timeout = 0;
  if (onClick)
  ...
  const handler = {
    updatePos: () => updatePos(contextElement, loading),
    show: () => {
      if (scheduled)
        return;
      scheduled = true;
...
// ../../node_modules/.pnpm/@innorules+web-api@2.0.4/node_modules/@innorules/web-api/lib/esm/builder-api/builder-client.js
var fetchHeaders = {
...
function createBuilderClient(clientConfig) {
  ...
  return {
    async request(options) {
      ...
      const header = Object.assign(Object.assign({ part, reqno: reqNo, serviceName }, DEFAULT_HEADER), options.header);
...
// ../../node_modules/.pnpm/@innorules+web-api@2.0.4/node_modules/@innorules/web-api/lib/esm/session/session-util.js
...
function createUUID(type) {
  return v4_default().replace(/-/g, "").substring(0, type === "ssid" ? SSID_LENGTH : PAGE_LENGTH);
}
...
// ../../node_modules/.pnpm/@innorules+web-api@2.0.4/node_modules/@innorules/web-api/lib/esm/session/session-channel.js
var PAGE_KEY_PREFIX = "com.innorules.webapi.page-param";
function getPageKey(pageId) {
  return `${PAGE_KEY_PREFIX}@${pageId}`;
}
var getSessionChannel = (() => {
  ...
  return () => {
    ...
    return {
      postSetItem(key, value) {
        sessionStorage.setItem(key, value);
        sessionChannel === null || sessionChannel === void 0 ? void 0 : sessionChannel.postMessage({ type: "set-item", key, value });
      },
      postClear() {
        sessionStorage.clear();
        sessionChannel === null || sessionChannel === void 0 ? void 0 : sessionChannel.postMessage({ type: "clear" });
      },
      postRemoveItem(key) {
        sessionStorage.removeItem(key);
        sessionChannel === null || sessionChannel === void 0 ? void 0 : sessionChannel.postMessage({ type: "remove-item", key });
      },
...
// ../../node_modules/.pnpm/@innorules+web-api@2.0.4/node_modules/@innorules/web-api/lib/esm/session/open.js
var openNewWindow = ({ url, parameters, target, features }) => {
  if (!hasSessionIdInURL())
    initNewSessionId();
  const pageId = createUUID("page");
  const adjustedTarget = target === "$NEW_SESSION" ? pageId : target;
  if (!parameters || Object.keys(parameters).length === 0)
    return Promise.resolve(window.open(makeNewUrl(url), adjustedTarget, features));
  return new Promise((resolve, reject) => {
    return getSessionChannel().savePageParam(pageId, JSON.stringify(parameters)).then(() => {
      resolve(window.open(makeNewUrl(url, pageId), adjustedTarget, features));
    });
  });
};

// ../../node_modules/.pnpm/@innorules+web-api@2.0.4/node_modules/@innorules/web-api/lib/esm/session/common-channel.js
var getCommonChannel = (() => {
...
// ../../node_modules/.pnpm/@innorules+web-api@2.0.4/node_modules/@innorules/web-api/lib/esm/session-storage.js
...
function refreshIRM010(irm010) {
  postSetItem(ENTRY_PARAMETER_KEY, JSON.stringify(Object.assign(Object.assign({}, getEntryParam()), { irm010Response: Object.assign({}, irm010) })));
}
...
// ../../node_modules/.pnpm/@innorules+web-api@2.0.4/node_modules/@innorules/web-api/lib/esm/entry-script.js
async function createEntryScriptLoader({ portalVersion, service, env, i18n, irm010Response, userId, services }) {
  ...
  const module = await import(`${serviceEnv.path}/${serviceEnv.appEntryPath}?t=${(/* @__PURE__ */ new Date()).getTime()}`);
  const param = {
    userId,
...
// ../../node_modules/.pnpm/@innorules+web-api@2.0.4/node_modules/@innorules/web-api/lib/esm/iframe-module/index.js
...
${webModuleValidationMsg.join("\n")}`);
  ...
  const callInitSession = async () => {
    const appEntryPath = `${service.path}/${service.appEntryPath}?t=${Date.now()}`;
    const entryModule = await import(appEntryPath);
    ...
    if (initSessionResult && initSessionResult.code !== 0) {
      console.error(initSessionResult.message);
      throw new Error(`initSession error! check the error logs.`);
    }
  };
  const getIFrameInstance = (() => {
    ...
    return async () => {
      ...
      const moduleParam = Object.assign(Object.assign({}, option.param), {
...
// ../../node_modules/.pnpm/zod@3.22.1/node_modules/zod/lib/index.mjs
var util;
(function(util2) {
...
// ../../node_modules/.pnpm/@innorules+web-api@2.0.4/node_modules/@innorules/web-api/lib/esm/zod-types.js
...
var getServiceInfoSchema = (serviceId) => z.object({
  ...
  appEntryPath: z.string().default("app-entry.js"),
...
// ../../node_modules/.pnpm/@innorules+web-api@2.0.4/node_modules/@innorules/web-api/lib/esm/env-init.js
...
// ../../node_modules/.pnpm/@innorules+web-api@2.0.4/node_modules/@innorules/web-api/lib/esm/version.js
...
var createTooltip = (options) => {
  ...
  let tooltipRefElement = refElement;
  const updateTooltipOffset = () => {
    computePosition(tooltipRefElement, tooltipElement, {
      // Tooltip 배치 위치
      placement,
      strategy: "fixed",
  ...
  const mouseMoveHandler = (ev) => {
    tooltipRefElement = getVirtualEl(ev.clientX, ev.clientY);
    updateTooltipOffset();
  };
  const showTooltip = () => {
    if (text === "") {
      console.warn("tooltip text is empty!", refElement);
      return;
    }
    updateText(text);
  ...
  const mouseEnterHandler = refMode === "element" ? showTooltip : showTooltipPointer;
  refElement.addEventListener("mouseenter", mouseEnterHandler);
  refElement.addEventListener("mouseleave", hideTooltip);
  return {
    destroy() {
      hideTooltip();
      refElement.removeEventListener("mouseenter", mouseEnterHandler);
...
var createTooltipBySelector = (options) => {
  ...
  return {
    destroy: () => {
      tooltipList.forEach(({ handler }) => handler.destroy());
    },
    updateTextAll() {
      tooltipList.forEach(({ handler, element }) => {
        const newText = element.dataset["title"] ?? "";
        if (handler.text !== newText)
          handler.updateText(element.dataset["title"] ?? "");
      });
    },
    updateTextOne(element, text) {
      const findItem = tooltipList.find((tooltipItem) => element === tooltipItem.element);
      if (!findItem) {
      ...
      if (text) {
        ...
        findItem.handler.updateText(text);
      } else {
        findItem.handler.updateText(element.dataset["title"] ?? "");
      }
    }
  };
};
...
function createSplitContainer(elements, options) {
  const splitHandler = split_es_default(elements, {
    ...options,
    snapOffset: (options == null ? void 0 : options.snapOffset) ?? 10,
    dragInterval: (options == null ? void 0 : options.dragInterval) ?? 5,
    gutterSize: 6
  });
...
function getTextWidthContext(fontSize, fontFamily) {
  ...
  return {
    // whitespace 공백 크기 문제로 |로 치환하여 너비 계산
    calculateWidth: (text) => Math.ceil(context.measureText(text.replace(/ /g, "|")).width)
  };
}
...
//# sourceMappingURL=chunk-RFSANBZJ.js.map
